...
function PartiesContent() {
  ...
  const endpoint =
    typeFilter === 'ALL' ? '/parties' : `/parties?type=${typeFilter}`;
  ...
  return (
    <div>
      <Header
        title="Parties"
        subtitle="Manage staff, vendors, and contacts for salary and expense entries"
      />
            ...
            {filtered.map((party) => (
              <Card key={party.id}>
                <CardContent className="p-6">
                  ...
                  {party.phone && (
                    <p className="text-sm text-muted-foreground mt-1">{party.phone}</p>
                  )}
                  {party.notes && (
                    <p className="text-xs text-muted-foreground mt-2 line-clamp-2">{party.notes}</p>
                  )}
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Button variant="outline" size="sm" onClick={() => startEdit(party)}>
